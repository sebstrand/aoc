...
cost = namedtuple('cost', ['ore', 'clay', 'obsidian' ], defaults=(0, 0, 0))
start_minutes = 18
...
def solve(input_file, p1=True):
    ...
    for bid, oro, cro, bro, brc, gro, grb in map(ints, lines):
        blueprints.append((
            bid,
            {
                'geode': cost(ore=gro, clay=0, obsidian=grb),
                'obsidian': cost(ore=bro, clay=brc, obsidian=0),
                'clay': cost(ore=cro, clay=0, obsidian=0),
                'ore': cost(ore=oro, clay=0, obsidian=0),
            }
        ))
